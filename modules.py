...
def accel(i, x, v, soundspeed, dens, Press, smooth, nneigh, neighlist, mpart):
    ...
    vdotr = dx * dv
    cab = 0.5 * (soundspeed[i] + soundspeed[neighbors])
    rhoab = 0.5 * (dens[i] + dens[neighbors])
    muab = (h * dv * dx)/(dx**2 + eta**2)
    Piab = np.where(vdotr < 0, (-1 * cab * muab + 2 * muab**2)/rhoab, 0.0)
    
    # Compute the pressure gradient contribution
    press_term = (
        Press[neighbors] / (dens[neighbors]**2) + Press[i] / (dens[i]**2) + Piab
        )
...
def energy(i, x, v, soundspeed, dens, Press, smooth, nneigh, neighlist, mpart):
    ...
    mask = (q <= 2.0) & (neighbors != i)
    contrib = press_term * dv * dx * gradkernel(q, h) / (np.abs(dx))
    dedt = 0.5 * np.sum(mpart * contrib[mask])
